...
const IteratorArgument_1 = require("./Argument/IteratorArgument");
const RewindableGenerator_1 = require("./Argument/RewindableGenerator");
const ServiceClosureArgument_1 = require("./Argument/ServiceClosureArgument");
const ServiceLocatorArgument_1 = require("./Argument/ServiceLocatorArgument");
...
const DefaultParameterBag_1 = require("./ParameterBag/DefaultParameterBag");
const EnvPlaceholderParameterBag_1 = require("./ParameterBag/EnvPlaceholderParameterBag");
const ResolveEnvPlaceholdersPass_1 = require("./Compiler/ResolveEnvPlaceholdersPass");
...
class ServiceNotFoundException extends Error {
    constructor(id) {
        super((0, sprintf_1.default)("Service with id or alias \"%s\" was not found", id));
    }
}
class ServiceCircularReferenceException extends Error {
    constructor(message, seen) {
        super(message);
    }
}
class ContainerBuilder extends Container_1.Container // implements TaggedContainerInterface
 {
    constructor(parameterBag = null) {
         ...
         */
        this.removedBindingIds = [];
        const deprecationMessage = "Depication %alias_id% Message";
        // this.trackResources = interface_exists(ResourceInterface::class);
        this.setDefinition('service_container', (new Definition_1.Definition(Container_1.Container)).setSynthetic(true).setPublic(true));
        ...
        this.setAlias(Container_1.Container.name, new Alias_1.Alias('service_container', false)).setDeprecated('symfony/dependency-injection', '5.1', deprecationMessage);
    }
    /**
    ...
    getExtension(name) {
        ...
        throw new Container_1.RuntimeException((`Container extension "${name}" is not registered.`));
    }
    /**
     ...
     * @final
     */
    getReflectionClass(serviceClass, _throw = true) {
        if (!(serviceClass = this.getParameterBag().resolveValue(serviceClass))) {
            return null;
        }
        if (serviceClass in ContainerBuilder.INTERNAL_TYPES) {
            return null;
        }
        throw new Container_1.RuntimeException("ReflectionClass not Supported");
    ...
    loadFromExtension(extension, values) {
        if (this.isCompiled()) {
            throw new Container_1.RuntimeException('BadMethodCallException: Cannot load from an extension on a compiled container.');
        }
        var namespace = this.getExtension(extension).getAlias();
        this.extensionConfigs[namespace].push(values !== null && values !== void 0 ? values : []);
    ...
    set(id, service) {
        if (this.isCompiled() && (id in (this.definitions) && !this.definitions[id].isSynthetic())) {
            ...
            throw new Container_1.RuntimeException(`BadMethodCallException: Setting service "${id}" for an unknown or non-synthetic service definition on a compiled container is not allowed.`);
        }
        delete this.definitions[id], this.aliasDefinitions[id], this.removedIds[id];
     ...
     * @see Reference
     */
    get(id, invalidBehavior = Container_1.InvalidServiceBehavior.EXCEPTION_ON_INVALID_REFERENCE) {
        if (this.isCompiled() && (id in this.removedIds) /* ContainerInterface::EXCEPTION_ON_INVALID_REFERENCE >= invalidBehavior */) {
            return super.get(id);
        }
        return this.doGet(id, invalidBehavior);
    }
    doGet(id, invalidBehavior = Container_1.InvalidServiceBehavior.EXCEPTION_ON_INVALID_REFERENCE, inlineServices = {}, isConstructorArgument = false) {
        var definition;
        var service;
        ...
        try {
            if (Container_1.InvalidServiceBehavior.IGNORE_ON_UNINITIALIZED_REFERENCE === invalidBehavior) {
                return super.get(id, invalidBehavior);
            }
            if (service = super.get(id, Container_1.InvalidServiceBehavior.NULL_ON_INVALID_REFERENCE)) {
                return service;
            }
        }
        catch (e) {
        ...
        try {
            ...
            if (Container_1.InvalidServiceBehavior.EXCEPTION_ON_INVALID_REFERENCE.valueOf() < invalidBehavior.valueOf()) {
                return null;
            }
            throw e;
        }
        ...
        if (definition.hasErrors() && (errors = definition.getErrors()).length > 0) {
            throw new Container_1.RuntimeException((errors.toString()));
        }
        if (isConstructorArgument) {
    ...
    merge(container) {
        if (this.isCompiled()) {
            throw new Container_1.RuntimeException('BadMethodCallException: Cannot merge on a compiled container.');
        }
        this.addDefinitions(container.getDefinitions());
        this.addAliases(container.getAliases());
        this.getParameterBag().add(container.getParameterBag().all());
        if (this.trackResources) {
            throw new Container_1.RuntimeException("Resources are not Supported");
        ...
        var envPlaceholders;
        var bag;
        if ((bag = this.getParameterBag()) instanceof EnvPlaceholderParameterBag_1.EnvPlaceholderParameterBag && container.getParameterBag() instanceof EnvPlaceholderParameterBag_1.EnvPlaceholderParameterBag) {
        ...
        for (const _interface in enteries) {
            ...
            if ((_interface in this.autoconfiguredInstanceof)) {
                throw new Container_1.InvalidArgumentException(`"${_interface}" has already been autoconfigured and merge() does not support merging autoconfiguration for the same class/interface.`);
            }
        ...
        for (const attribute in enteries) {
            ...
            if ((attribute in this.autoconfiguredAttributes)) {
                throw new Container_1.InvalidArgumentException(`"${attribute}" has already been autoconfigured and merge() does not support merging autoconfiguration for the same attribute.`);
            }
     ...
     */
    compile(resolveEnvPlaceholders = true) {
        ...
        if (resolveEnvPlaceholders && bag instanceof EnvPlaceholderParameterBag_1.EnvPlaceholderParameterBag) {
            // console.log('added ResolveEnvPlaceholdersPass: ', ResolveEnvPlaceholdersPass);
            compiler.addPass(new ResolveEnvPlaceholdersPass_1.ResolveEnvPlaceholdersPass(), PassConfig_1.PassHookPoint.AFTER_REMOVING, -1000);
        }
        ...
        if (bag instanceof EnvPlaceholderParameterBag_1.EnvPlaceholderParameterBag) {
            if (resolveEnvPlaceholders) {
                this.parameterBag = new DefaultParameterBag_1.DefaultParameterBag(this.resolveEnvPlaceholders(bag.all(), true));
            }
        ...
        for (const id in defs) {
            const definition = this.definitions[id];
            if (!(definition instanceof Definition_1.Definition))
                continue;
    ...
    setAlias(alias, id) {
        ...
        // }
        if ((typeof id === 'string')) {
            id = new Alias_1.Alias(id);
        }
        else if (!(id instanceof Alias_1.Alias)) {
            throw new Container_1.InvalidArgumentException('id must be a string, or an Alias object.');
        }
        if (alias === String(id)) {
            throw new Container_1.InvalidArgumentException(`An alias cannot reference itself, got a circular reference on "${alias}".`);
        }
    ...
    getAlias(id) {
        if (!(id in this.aliasDefinitions)) {
            throw new Container_1.InvalidArgumentException((`The service alias "${id}" does not exist.`));
        }
    ...
    register(id, serviceClass) {
        return this.setDefinition(id, new Definition_1.Definition(serviceClass));
    }
    /**
     ...
     * @return Definition
     */
    autowire(id, serviceClass) {
        return this.setDefinition(id, (new Definition_1.Definition(serviceClass)).setAutowired(true));
    }
    /**
    ...
    setDefinition(id, definition) {
        if (this.isCompiled()) {
            throw new Container_1.RuntimeException('BadMethodCallException: Adding definition to a compiled container is not allowed.');
        }
     ...
     * @important
     */
    createService(definition, inlineServices, isConstructorArgument = false, id = null, tryProxy = true) {
        var _a;
        var factory;
        var service;
        var proxy;
        ...
        if (definition.isSynthetic()) {
            throw new Container_1.RuntimeException(`You have requested a synthetic service ("id"). The DIC does not know how to construct this service.`);
        }
        ...
        if (tryProxy && definition.isLazy() && !(tryProxy = !(proxy = this.proxyInstantiator)) /*|| proxy instanceof RealServiceInstantiator*/) {
            throw new Container_1.RuntimeException("Ended up in a service proxy while creating");
        ...
        if (null !== factory && undefined !== factory) {
            ...
            service = Reflect.construct(definition.getClass(), args);
        ...
        if (callable = definition.getConfigurator()) {
            if (Array.isArray(callable)) {
                ...
                if (callable[0] instanceof Reference_1.Reference) {
                    callable[0] = this.doGet(String(callable[0]), callable[0].getInvalidBehavior(), inlineServices);
                }
                else if (callable[0] instanceof Definition_1.Definition) {
                    callable[0] = this.createService(callable[0], inlineServices);
                }
            }
            if (!(typeof callable == 'function')) {
                throw new Container_1.InvalidArgumentException(`The configure callable for class "${service}" is not a callable.`);
            }
            callable(service);
        }
        return service;
    ...
    doResolveServices(value, inlineServices = {}, isConstructorArgument = false) {
        ...
        if (value && (typeof value == 'object' && (value.constructor === Object || value instanceof Array))) {
        ...
        else if (value instanceof ServiceClosureArgument_1.ServiceClosureArgument) {
        ...
        else if (value instanceof IteratorArgument_1.IteratorArgument) {
            value = new RewindableGenerator_1.RewindableGenerator(function* () {
                ...
                level_1: for (const k in values) {
                    ...
                    for (const s of ContainerBuilder.getInitializedConditionals(v)) {
                        if (!self.doGet(s, Container_1.InvalidServiceBehavior.IGNORE_ON_UNINITIALIZED_REFERENCE, inlineServices)) {
                            continue level_1;
                        }
                    }
                ...
                level_1: for (const k in values) {
                    ...
                    for (const s of ContainerBuilder.getInitializedConditionals(v)) {
                        if (!self.doGet(s, Container_1.InvalidServiceBehavior.IGNORE_ON_UNINITIALIZED_REFERENCE)) {
                            continue level_1;
                        }
                    }
                    ++count;
        ...
        else if (value instanceof ServiceLocatorArgument_1.ServiceLocatorArgument) {
            var refs = [], types = [];
            var values = value.getValues();
            for (const k in values) {
                ...
                if (v) {
                    refs[k] = [v];
                    types[k] = (v instanceof TypedReference_1.TypedReference) ? v.getType() : '?';
                }
            }
            // value = new ServiceLocator(\Closure::fromCallable([this, 'resolveServices']), refs, types);
            throw new Container_1.RuntimeException("Not Class: ServiceLocator");
        }
        else if (value instanceof Reference_1.Reference) {
            value = this.doGet(String(value), value.getInvalidBehavior(), inlineServices, isConstructorArgument);
        }
        else if (value instanceof Definition_1.Definition) {
            value = this.createService(value, inlineServices, isConstructorArgument);
        }
        else if (value instanceof Parameter_1.Parameter) {
        ...
        // }
        else if (value instanceof AbstractArgument_1.AbstractArgument) {
            throw new Container_1.RuntimeException(value.getTextWithContext());
        }
        return value;
    }
    /**
        ...
        for (const id in definitions) {
            ...
            if (definition.hasTag(name)) {
                if (throwOnAbstract && definition.isAbstract()) {
                    throw new Container_1.InvalidArgumentException(`The service "%s" tagged "%s" must not be abstract.`);
                }
     ...
     */
    findUnusedTags() {
        const unusedTags = this.findTags().filter((tag) => !this.usedTags.includes(tag));
        return unusedTags;
     ...
     */
    registerForAutoconfiguration(_interface) {
        if (!(_interface in this.autoconfiguredInstanceof)) {
            this.autoconfiguredInstanceof[_interface] = new ChildDefinition_1.ChildDefinition('');
        }
     ...
     */
    registerAttributeForAutoconfiguration(attributeClass, configurator) {
     ...
     */
    registerAliasForArgument(id, type, name = null) {
        // name = (new Target(name ?? id)).name;
        name = name !== null && name !== void 0 ? name : id;
        if (!(/^[a-zA-Z_\x7f-\xff]/.test(name))) {
            throw new Container_1.InvalidArgumentException(`Invalid argument name "${name}" for service "${id}": the first character must be a letter.`);
        }
    ...
    resolveEnvPlaceholders(value, format = null, usedEnvs = null) {
        ...
        const envPlaceholders = (bag instanceof EnvPlaceholderParameterBag_1.EnvPlaceholderParameterBag) ? bag.getEnvPlaceholders() : this.envPlaceholders;
        var completed = false;
        var resolved;
                ...
                if (RegExp(`${placeholder}`).test(value)) {
                    ...
                    if (placeholder === value) {
                        ...
                        if (!(typeof resolved == 'string') && !(typeof resolved == 'number')) {
                            throw new Container_1.RuntimeException(`A string value must be composed of strings and/or numbers, but found parameter "env(${env})" of type "${typeof resolved}" inside string value "${this.resolveEnvPlaceholders(value)}".`);
                        }
     ...
     */
    static willBeAvailable(_package, serviceClass, parentPackages) {
        const skipDeprecation = 3 < arguments.length && arguments[3];
    ...
    static getServiceConditionals(value) {
        ...
        else if (value instanceof Reference_1.Reference && Container_1.InvalidServiceBehavior.IGNORE_ON_INVALID_REFERENCE === value.getInvalidBehavior()) {
    ...
    static getInitializedConditionals(value) {
        ...
        else if (value instanceof Reference_1.Reference && Container_1.InvalidServiceBehavior.IGNORE_ON_UNINITIALIZED_REFERENCE === value.getInvalidBehavior()) {
            services.push(String(value));
        }
        return services;
    }
     ...
     */
    static hash(value) {
        throw new Container_1.RuntimeException("BadMethodCallException: Hash not implemented on containers");
    ...
    getEnv(name) {
        ...
        const bag = this.getParameterBag();
        if (!(typeof value == 'string') || !(bag instanceof EnvPlaceholderParameterBag_1.EnvPlaceholderParameterBag)) {
            return value;
        }
    ...
    callMethod(service, call, inlineServices) {
        ...
        for (const s of ContainerBuilder.getInitializedConditionals(call[1])) {
            if (!this.doGet(s, Container_1.InvalidServiceBehavior.IGNORE_ON_UNINITIALIZED_REFERENCE, inlineServices)) {
                return service;
            }
        }
        const result = service[call[0]](...this.doResolveServices(this.getParameterBag().unescapeValue(this.getParameterBag().resolveValue(call[1])), inlineServices));
     ...
     * @param mixed service
     */
    shareService(definition, service, id, inlineServices) {
    ...
    getExpressionLanguage() {
        throw new Container_1.RuntimeException("BadMethodCallException: getExpressionLanguage()");
...
ContainerBuilder.INTERNAL_TYPES = {
    'int': true,
    'float': true,
    'string': true,
    'bool': true,
